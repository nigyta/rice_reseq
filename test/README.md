...
2. Read mapping and BAM conversion (fastq2bam.cwl)  
    ...
    2.7 Calculate average depth from de-duplicated BAM (samtools depth, in-house script) __[depth.txt]__  
    2.8 Create BAM index (samtools index) __[BAM index (.bai)]__  
    2.9 Statistics for de-duplicated BAM (samtools stats) __[stats.txt]__  
    2.10 Extract unmapped reads from BAM (samtools, picard) __[FASTQ]__  
...
| {outprefix}.rmdup.bam.avg-depth.txt | Average depth file for de-duplicated BAM (generated using samtools-depth) | 2.7 |
| {outprefix}.rmdup.bam.stats.txt | Statistics for de-duplicated BAM (generated using samtools-depth, in-house script) | 2.9 |
| {outprefix}.unmapped-read.{r1/r2}.fastq.gz | Reads not mapped to the reference genome | 2.10 |
